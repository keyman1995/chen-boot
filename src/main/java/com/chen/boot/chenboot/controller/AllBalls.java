...
public class AllBalls {
    ...
    private static void getTotalSalesContent(String pageContent) {
        String regex = "<td><strong>[\\d]+?\\,[\\d]+?\\,[\\d]+?</strong></td>";
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(pageContent);
        while (matcher.find()){
            String content = matcher.group();
            getTotalSales(content);
        }
    }

    private static void getTotalSales(String content) {
        String regx = ">[\\d]+?\\,[\\d]+?\\,[\\d]+?<";
        Pattern pattern = Pattern.compile(regx);
        Matcher matcher = pattern.matcher(content);
        while (matcher.find()){
            String totalSalse = matcher.group();
            totalSalse = totalSalse.substring(1, totalSalse.length()-1);
            System.out.println(totalSalse);
        }
    }

    public static void main(String[] args) {
        getTotalSalesContent("fdsgfdhgfdjhgkjkljh<td><strong>334,319,180</strong></td>fdsgffdgfdsgfsdgfdg");
